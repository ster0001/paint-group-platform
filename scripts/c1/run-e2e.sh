...
echo "Starting the C1 dev server on :3101 (test stack)…"
npm run dev -- -p 3101 >/tmp/pg-c1-dev.log 2>&1 &
DEV_PID=$!
...
for i in $(seq 1 60); do
  ...
  sleep 1
  if [ "$i" = 60 ]; then echo "Dev server never came up — see /tmp/pg-c1-dev.log"; exit 1; fi
done

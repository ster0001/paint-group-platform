...
- **R4 (2026-08-17), in order: `20260905000000_upload_limits.sql`, `20260906000000_bank_change_alert.sql`, `20260907000000_work_order_contractor_index.sql`.** Paste all three together. Until they run: uploads have no server-side size or type limit, a change to a contractor’s bank details raises no alert, and `work_orders.contractor_id` has no index. The app degrades quietly without them (no banner, old upload behaviour) — see `docs/manual-tests/r4-uploads-and-alerts.md`.

...
// run ~15 s. One place to tune.
const HOLD_BEFORE_LOOP_MS = 4500;
...
/** The beats, tuned 20 Sep (mockup was 0.7 / 3 / 0.32 / 1.4 / 3.2 / 2.7): first text after 0.5 s, read 1.9 s, tap 0.28 s, app opens 0.9 s, first item 2 s, then 1.7 s each. */
function delayAfter(step: number, n: number): number {
  if (step === 0) return 500;
  if (step === 1) return 1900;
  if (step === 2) return 280;
  if (step === 3) return 900;
  const k = step - STEP_APP; // items shown so far
  return k === 1 ? 2000 : k <= n ? 1700 : 0;
}

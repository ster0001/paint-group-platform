...
/**
 * The Customer tab (session 2.2).
 *
 * With no `id` it lists the customers; with one it shows that customer — the
 * mockup's header, the worked-out numbers, "Log something", and "Everything, in
 * order". The list is a stand-in for the board, which arrives in 2.3 and
 * becomes the way in.
 */
export default async function CrmCustomerPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  if (!id) {
    const { data: accounts } = await supabase
      .from("accounts").select(columns).order("created_at", { ascending: false }).limit(200);
    const rows = (accounts ?? []) as AccountRow[];

    // What each customer is worth and when they were last quoted, in one read
    // rather than one per card.
    const { data: estimates } = await supabase
      .from("estimates").select("account_id, total_cents, created_at, status")
      .not("account_id", "is", null).limit(500);
    const byAccount = new Map<string, { value: number; last: string | null }>();
    for (const e of (estimates ?? []) as Array<{ account_id: string; total_cents: number | null; created_at: string }>) {
      const cur = byAccount.get(e.account_id) ?? { value: 0, last: null };
      cur.value += e.total_cents ?? 0;
    ...
    return (
      <>
        <h2>Customers</h2>
        <p className="sub">
          Everyone with an account. The board — lanes, counts, what needs you today — lands next;
          this is the way in until then.
        </p>
        {rows.length === 0 ? (
          <p className="empty">No customers yet. An account is created the first time an estimate is saved with an email.</p>
        ) : (
          <div className="people">
            {rows.map((a) => {
              const agg = byAccount.get(a.id);
              const days = daysSince(agg?.last ?? null);
              return (
                <Link key={a.id} className="person" href={`/crm?id=${a.id}`}>
                  <span className="cname">
                    {a.temperature && <i className={`dot ${a.temperature}`} aria-hidden="true" />}
                    {a.name || a.email}
                  </span>
                  <span className="cmeta">
                    {a.account_type === "trade" ? "Trade account" : "Residential"}
                    {a.phone ? ` · ${a.phone}` : ""}
                  </span>
                  <span className="cfoot">
                    <b className="cval">{agg?.value ? money(agg.value) : "—"}</b>
                    <span className="cwhen mono">{days == null ? "no estimate" : days === 0 ? "today" : `${days}d`}</span>
                  </span>
                </Link>
              );
            })}
  ...
  if (!account) {
    return (
      <>
        <Link className="back" href="/crm">← Customers</Link>

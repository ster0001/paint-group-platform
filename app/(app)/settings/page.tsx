...
import { rewardsFromSettings, WIZARD_REWARDS_KEY } from "@/lib/wizard/ladder";
...
export default async function SettingsPage() {
  ...
  // PR 1 of the tiers plan: the thresholds and caps were SQL-only until now.
  const tiers = {
    ...
    rewards: rewardsFromSettings(allSettings.find((r) => r.key === WIZARD_REWARDS_KEY)?.value),
  };
  ...
  const buckets: SettingsBucketDef[] = [
    ...
    },
    {
      ...
      folders: [
        ...
        { id: "tiers", title: "Tiers & rewards", subtitle: `Silver from ${tiers.bands.midMin}% · Gold from ${tiers.bands.tightMin}% · online cap $${Math.round(tiers.policy.interiorSelfServeCapCents / 100).toLocaleString("en-AU")} inside${tiers.rewards.goldSkipVisit ? " · Gold books straight in" : ""}`,

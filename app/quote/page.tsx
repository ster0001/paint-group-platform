...
export default async function QuotePage({
  ...
  const [rateItems, modifiers, products, settings, lineItems, areaNames, contactsRes, estimateRes, workOrderRes, contractorsRes, presentationsRes, typicalRes] = await Promise.all([
    ...
    id ? supabase.from("estimates").select("id, title, builder_state, share_token, status, sent_at, viewed_at, accepted_at, valid_until, presentation_id, sent_snapshot").eq("id", id).single() : Promise.resolve({ data: null }),
  ...
  type Initial = { id: string | null; title: string | null; builder_state: unknown; share_token?: string | null; status?: string | null; sent_at?: string | null; viewed_at?: string | null; accepted_at?: string | null; valid_until?: string | null; presentation_id?: string | null };

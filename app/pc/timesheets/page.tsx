...
export default async function TimesheetsPage() {
  ...
  const [pending, decided, employees, assignments, rates, leave] = await Promise.all([
    ...
    supabase.from("wo_assignments").select("contractor_id, work_order_id, start_date, end_date, work_orders(wo_ref, wo_snapshot, stage)").neq("status", "released"),
    supabase.from("employee_cost_rates").select("contractor_id, effective_from").lte("effective_from", today),
    // S7: leave / RDO requests still undecided (sick days count at once and are not asked).
    supabase.from("contractor_unavailability")
  ...
  type AssignRow = { contractor_id: string; work_order_id: string; start_date: string; end_date: string; work_orders: { wo_ref: string; wo_snapshot: unknown; stage: string } | null };
  ...
  // Time off requests, with any booked days they land on named up front.
  type LeaveReq = {
    id: string; contractor_id: string; kind: "leave" | "rdo"; start_date: string; end_date: string; reason: string; created_at: string;
    contractors: { profiles: { name: string | null } | null } | null;
  };
  const leaveRows: LeaveRowProp[] = ((leave.data ?? []) as unknown as LeaveReq[]).map((r) => ({
    id: r.id, painter: r.contractors?.profiles?.name?.trim() || "Employee", kind: r.kind,
    start: r.start_date, end: r.end_date, reason: r.reason, requestedAt: r.created_at,
    clashes: assignRows
      .filter((a) => a.contractor_id === r.contractor_id && a.work_orders && a.work_orders.stage !== "closed")
      .filter((a) => a.start_date <= r.end_date && a.end_date >= r.start_date)
  ...
  return (
    <>
      ...
      <div className="sect" id="time-off">
        <div className="sect-h"><h2>Time off requests</h2><span>leave · RDO</span></div>
        <div className="stack" data-testid="leave-requests">
          {leaveRows.map((row) => <LeaveRow key={row.id} {...row} />)}
          {leaveRows.length === 0 && (
            <p className="empty" data-testid="leave-empty">Nothing asked for. A sick day never waits here — it counts at once and raises Reassign on any booked day.</p>
          )}
        </div>
      </div>

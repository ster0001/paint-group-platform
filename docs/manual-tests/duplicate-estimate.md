...
The title is the same as the original; the first line of the address reads the old street with " (copy)" on the end. Every
...
Expect: a new **draft** row at the top with the original's title and
"<street> (copy), <suburb>" under it. Open it: everything from step 1
is still there.
...
Press Duplicate on the copy. Expect a row whose address reads "… (copy 2)".

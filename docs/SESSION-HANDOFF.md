...
with Mine/Team (sales-only login defaults to Mine; export carries `who`), the target card (Settings target,
pace, "no target set", twelve-month one-axis bar chart with table toggle — dataviz method, contrast relief),
...
ageing 1–7/8–30/31+, unsent pipeline by stage, days-to-pay; tripwire vs the /invoicing pulse tiles).
...
seed (measured once in the spec log; the gate is session 6). Gates: vitest 2749/2749 (core on the golden seed, strip, roles, the loop's own offset audit — which caught a written +10:00 in the seed; instants now come from melbourneInstant), tsc + eslint clean. e2e `dashboard-home.spec.ts` 5/5 green on :3103 (four logins → four homes; finance's sales export 403; tile = CSV rows; chips) — ran once the CI lock freed (~13:50 UTC); two harness fixes on the way (Playwright's bare `request` fixture carries no cookies → use `page.request`; a CSV header with a comma is quoted). Home rendered for the master in ~5 s on the test project's 66k estimates — the 1.5 s gate is session 6 (the work queue + console loads dominate; cache or narrow them). 20270180 applied on TEST via reapply-one.
...
column the master ticks on Staff logins; `lib/reporting/roles.ts` = effective roles, union of sections,
who sees money (⚑1/⚑2 pinned by roles.test.ts). Settings → Dashboard folder (owner/admin only): monthly
target ($ inc GST, month total — ⚑9), marketing spend by channel (⚑5), thresholds (⚑7 3 days, ⚑8 25%,
ageing 7/30). Not in 0d: the lead-source list rename (⚑4, CRM Phase 0) and payment methods shown
(invoicing session). Gates: vitest 2732/2732, tsc + eslint clean (4 pre-existing warnings), `e2e/dashboard-settings-roles.spec.ts` 5/5 green on :3103 (master = all five roles; sales = sales only, no dollars; RLS: sales reads an empty list and gets 42501 on insert, guard refuses self-promotion; roles ticked on Staff logins add up; target/spend/threshold saved on the Dashboard folder; sales never sees the folder). 20270179 applied on TEST via reapply-one and read back. The run waited ~50 min for the lock a CI run on main held — Tom's rule from tonight: wait once and report, never loop.
Prod: 20270179 needs Tom's paste (lock_timeout first; ledger row before merge). Next: SESSION 1 — reporting
core (`lib/reporting/`, PeriodMetric/NowMetric, golden seed, metrics_daily + cron, export route, `/home`
shell + needs-doing strip; e2e: four logins land on four homes, sales calling P&L gets 403).

# 19 Sep 2026 (night) — Home dashboard v2, Session 0c: capture on work orders. ONE migration: 20270178 (additive: `all_surfaces_done` event + backfill, `booking_offers.booked_end_date` + `booking_extended` trigger, `wo_qa_checks.attempt_no` + trigger/backfill, `review_requests` table + 2 RPCs, `contractors.capture_worked_hours` + setter, `wo_worked_hours` table + `wo_enter_worked_hours`, settings `worked_day_hours` + `worked_day_hours()`). Branch `feat/home-dashboard-0c-workorders`.

Built: the painter's opt-in hours question on the Finish card (pre-filled from the booking, skippable, server refuses `not_asked` for anyone not opted in), the "Asks for hours" button per painter on /contractors, the self-invoice reference row, the PC ReviewCard (walkthrough + closed), and `lib/reporting/workedTime.ts` — the ONE blended-hours function (entered verbatim, else schedule × day length, stops at the last tick, capped by the hours allowance; coverage line; calibration = entered only). Verified, not built: actor on photos/expenses/messages/events already exists. NOT built: `wo_callbacks` (trade portal v2 session 5), employee timesheets as an hours source (approved `timesheet_entries` could feed `entered` later — ⚑ for Tom), the review-request automation (still `planned`). Gates: vitest 2722/2722, tsc + eslint clean (4 pre-existing warnings), `e2e/dashboard-capture-workorders.spec.ts` 6/6 green on :3103 (as the painter: flag off → no question + `not_asked`; flag on → asked, pre-filled, entered; all_surfaces_done once; booking_extended on a grown span only; attempt_no 1→2→2; review asked/received; /contractors button flips the flag). 20270178 applied on TEST via reapply-one and read back (3 columns, 2 tables, 3 policies, 3 triggers, 5 functions, day hours 8). Environment: the Mac's swap was full (16.8/17 GB) — one `next build` hung at 0% CPU and had to be killed; peers held :3101 and the run lock for 10+ min; served on :3103 and looped on the lock.
Prod: 20270178 needs Tom's paste — start it with `set lock_timeout = '15s';` and compare the read-back row to its `_expect_` values; check `_prod_migrations` before merging (the 0a lesson). Next: 0d Settings and roles (B7 — `staff_profiles.staff_role`, Settings → Team, `sales_targets`, `marketing_spend`, threshold settings).

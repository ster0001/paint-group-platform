...
## One ladder — PR 1 of the reward tiers (8 Sep 2026)

`lib/wizard/ladder.ts` is now the only place "what tier is this, can they accept online, why is a person involved, what would lift it" is decided. Before it, the self-serve verdict was copied out three times (`lib/wizard/customer-scope.ts`, the wizard-edit route, `lib/agent/scope-tools.ts checkThresholds`), each reading `scope_editor.selfServe*` with hard-coded fallbacks, on top of `evaluateGuardrails` — whose `canAccept` already carried the `wizard_policy` caps and the accuracy bar. **Deleted:** the three formulas, the `scope_editor.selfServe*` keys (migration `20270132` strips them and the v1 `wizard_policy` keys), the two local `Ladder` types in the editors.

`ladderFor()` → `{ tier, selfServe, reason, visitSlots, nextUnlock }`. Tier = the `wizard_bands` thresholds (Bronze < `midMin`, Silver ≥ `midMin`, Gold ≥ `tightMin` **and** `decision.canAccept`); `selfServe` is exactly `decision.canAccept`; `reason` is `visitReason` when not self-serve. `nextUnlock` never names a target the road can't reach: exterior, mixed scope, over the cap, a hand-off, or a visit-only reason (peeling / rot / flagged / photos) returns null; a no-plan interior at Silver is told "upload your floorplan". Trap found by its own test: `visitReason` returns `"signoff"` as its residual for *any* non-self-serve job, so treating "signoff" as visit-only hid Gold from the one job that can reach it — "signoff"/"big" are deliberately outside `VISIT_ONLY`.

Surfaces: both editors show a `tier-chip` and a `tier-next` line on the confidence card; `checkThresholds` returns `tier` + `nextUnlock` for the assistant. **Settings → Estimates → Tiers & rewards** (`TiersSettings.tsx`) edits `wizard_bands`, `wizard_policy` (v2 keys) and the new `wizard_rewards` (`DEFAULT_REWARDS` = Tom's list; `goldSkipVisit` OFF). Tests: `lib/wizard/ladder.test.ts` (11), `e2e/tiers-settings.spec.ts`, the ladder spec's chip assertions.

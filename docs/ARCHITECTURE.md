...
Tom: painters may upload videos as well as photos from the Variations and Photos & notes cards. No schema change — a video is told from a photo by its **storage path extension**: `POST /api/wo/photos` now takes the file's `contentType` and names a video object `.mp4` / `.mov` / `.webm` (`videoExtensionFor`, photo paths stay bare as every older row is), and the ingest reads only the first 64 bytes of the staged object through a signed URL + Range request (`headBytes`, so a 100 MB clip never passes through the function), then requires the bytes to agree with the name (`sniffVideoKind` in `lib/extract/normalise.ts` — ISO-BMFF video brands / `qt  ` / EBML — kept separate from `sniffKind` so plans and bills still refuse video). Size caps: photos 25 MB, videos `MAX_VIDEO_UPLOAD_BYTES` 200 MB; the wo-photos bucket's own limit applies underneath. `WOPhoto.media` (`isVideoPath`) drives `PhotoGrid` (poster `<video>` + play badge, `data-media`) and `PhotoLightbox` (`<video controls>`, streamed by signed URL); the customer's `lib/portal/photos.ts` signs a video without the image transform and the account grid plays it. Spec: `e2e/wo-photos.spec.ts` (a progress clip renders and opens); units in `normalise.test.ts` / `photos.test.ts`.

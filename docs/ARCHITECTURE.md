...
**On the estimate** (`/e/[token]`) a fourth trust card sits after public liability and the SWMS card — `snap.proof.warranty`, which every snapshot has carried since the beginning and nothing rendered — linking to a new `#warranty` section: the promise, what it does not cover, and the nine clauses folded behind "The warranty in full". The clause text is built from `snap.company`, so a token surface still renders **snapshot** data and an estimate sent last month shows the details it was sent with. The printed quote gets its own `print-warranty` block (promise, no-transfer, ACL line, pointer to the full terms), the same shape as the SWMS block beside it. No migration for any of this and no new snapshot field: the warranty is universal, so there is nothing per-estimate to store.
...
Specs: `e2e/estimate-warranty.spec.ts` (anonymous customer — the card's place in the trust row, the terms opening, no watermark, the no-transfer clause, the printed block) and the warranty assertions in `e2e/portal-aftercare.spec.ts`. Help: `docs/help/warranty/{customer,staff}.md`.

...
## Proving window — rows can be set aside (9 Sep 2026)

Tom asked for a delete button for proving rows that aren't a fair test (the window had been benchmarking the wizard against PaintScout quotes that themselves lost money). Nothing is deleted: a row is an estimate, often a real job. `builder_state.wizard.provingExcluded` (beside the correction tag — no column, no migration) carries `{ at, by, reason }`; `splitProving()` divides the rows and **the summary is computed from the kept ones alone**, so the median, the ±10% share and the gate all stop counting what was set aside. `setProvingExcludedAction` takes a list, so one row and "remove all" are the same action; the page shows a **Set aside · N** panel with per-row and bulk "Put back".

Two things learned building it, both in the code as comments: putting a row back **deletes** the key rather than writing JSON `null` (a JSON null still answers a Postgres `is null` filter as present, which sent the first cleanup script into a loop across the 65k-row test project), and the bulk write goes in parallel chunks of 25 — serial writes over a full 200-row window left the button saying "Saving…" for about a minute, because every save revalidates and re-prices the whole page. Tests: `lib/wizard/proving.test.ts` (+2), `e2e/proving-exclude.spec.ts` (2, as staff; it restores any row it set aside that wasn't its own).

...
## An employee reads the employee manual (19 Sep 2026)

Tom, with a photo of a painter's phone: "none of the walkthrough gif videos work on mobile in the contractor portal". Not mobile, and not the films — they serve 200 as `image/gif`, loop infinitely, and load in Chromium and WebKit at iPhone size against a real build. The screenshot named the fault: **PAINTER PORTAL**, an **Expenses** tab — an employee — reading the *contractor* scheduling guide, with a broken "Walkthrough" icon. Two places answered "which manual is this reader's" and disagreed. `app/portal/help/page.tsx` and `app/portal/help/[feature]/page.tsx` passed the LITERAL `"contractor"` to `rolesFor`/`readGuide`, so everyone in the portal got the contractor file; `app/api/help/media/[feature]/[file]` asks `helpReader()`, which resolves the reader's real role — `["employee"]`. The page therefore rendered `walkthrough: media/contractor-walkthrough.gif` and the media route refused it (`mediaPathFor` finds no index entry for that file in the employee's roles): 404, broken icon, on every guide that has a film — and all three films are contractor-only, hence "none of them". The broken image was the visible half; the other half was the content, because the guide an employee was being handed opens "Each offer comes with the dates, the calculated labour hours, **your price** and a 24-hour clock", and an employee never sees a figure (`painterCapabilities.seesMoney`). Both pages now take `employmentType` and `capabilities` from the already-cached `getContractorSession()` and pass them to `rolesFor` / `readGuide` / `loadTour`, so the page can never offer a picture the media route will refuse. The Help page's "Show me around again" also gets the gate the portal layout already applied to the first-run tour (`capabilities.acceptsOffers`) — the contractor tour says "your price", "your invoices", "send a progress claim". There is no `docs/help/_tours/employee.md` yet, so an employee now gets no tour rather than the wrong one. Spec: `e2e/employee-help.spec.ts` — the employee's list, a contractor-only guide 404ing for them, **every image on every employee guide actually loading** (the reported symptom), a contractor's film still loading for a contractor, and no pay money. `e2e/help-centre.spec.ts` counted the office's guides as staff + pc (13) against the 17 `/help` lists — since C17 the office also reads the three customer-facing estimator guides — a stale assertion fixed here too. No migration.

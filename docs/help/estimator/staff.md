...
sources: app/(app)/estimates/WaitingTable.tsx, app/(app)/estimates/actions.ts, app/quote/PackPane.tsx, app/quote/StripActions.tsx, app/api/confirmations/[id]/route.ts, lib/crm/work-queue.ts, app/quote/QuoteBuilder.tsx, lib/estimate/leadSource.ts, app/(app)/settings/PresentationsManager.tsx
...
- **The Materials card reads the client label.** Rename a surface for the customer (Edit Surface → **Client Label**, say "Feature wall" on a Walls row) and the Materials row for that surface type reads the same — the substrate's own name sits behind it on hover. Two surfaces of one type with different labels show both, "Feature wall / Hall walls".
- **Finding a paint.** The **Materials** card lists one product per surface type, and the catalogue is long, so the paints in every dropdown read **A–Z** and a **search box at the top of the card** narrows all of them at once — type "dulux", or "low sheen". Two things it never does: it never hides the paint a row is already set to, and it never changes what a row is quoted with. Clear the box to see the whole catalogue again.
- **Counts reach the customer.** On a per-item substrate row (doors, windows, posts, architraves — anything priced per item), **Count (items)** in the row's detail panel is what the customer sees: their copy reads **Doors × 4** in that room, on the page and on the PDF, and the same count sits in an "Optional extras" line when the row is an option. A count of 1 shows no chip; measured rows (walls, ceilings) show no count. Estimates sent before 18 Sep 2026 show counts after their next save.
- **Photos on the customer's copy.** The customer's page shows nine photos from the areas' photo boxes; when the estimate carries more, a **More photos (N)** button under the grid opens the rest.
- **Lead source is required before Send.** Under Job settings, above the Presentation picker, **Lead source** says where this job came from — Referral, Repeat customer, Paid Google, Organic search, Paid social, Social, Email, Sign or vehicle, Phone enquiry, Direct, Other, or Not recorded. Online estimates arrive with it filled from the customer's first visit to the site; estimates you build yourself start blank, and **Send** says "Pick where this lead came from" until you choose. Your pick is copied to the customer's account when the account has none, and it stays — a later estimate never changes it. "Not recorded" is allowed but never fills the account. The home dashboard's Sales and Marketing sections report by this.
- **Leave out of the dashboard.** Under Job settings, below Lead source, a tick box for a test job or a duplicate. Ticked, nothing on Home counts this estimate, its job, its invoices or its payments (every section at once); the estimate and the job stay where they are and the box says "Left out of the dashboard from now on." Untick to count it again. It saves on the spot, not with the estimate, and only staff can press it.
- **What the presentation does for reporting.** The presentation you tick also gives the estimate its **category** for the dashboard — by the presentation's *category label* (Settings → Presentations → Edit → **Category label**; blank means the same as the name). Two presentations can share a label, and rewriting one keeps its history. The first presentation sent to a customer becomes their account's category and is never overwritten; an estimate with no presentation reports as "Uncategorised".

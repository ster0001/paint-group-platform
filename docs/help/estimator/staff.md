...
sources: app/(app)/estimates/WaitingTable.tsx, app/quote/PackPane.tsx, app/quote/StripActions.tsx, app/api/confirmations/[id]/route.ts, lib/crm/work-queue.ts
---
...
2. Open the row → the estimate opens on its **Pack** tab: the customer's answers, the range they saw, the rooms and what was assumed (amber) versus confirmed by them (cyan), their photos, and any notes from the assistant.
3. The strip has three buttons:
...
4. A fixed price cannot be fixed again — that is a variation. A visit already booked blocks a second booking until it is done or cancelled.
5. **Commercial requests** never reach fix-online. A brief request (strata, hospital, any outside work) arrives with the brief on the pack and a visit already booked from the customer's slot pick; the site checklist items it raised (induction, meeting date, hazmat) are on the Pack too.
6. **Trade requests** likewise always come to you. Fixing one puts the tree on the property's file; the agent's next quote on that property is seeded from it.

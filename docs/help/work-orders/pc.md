...
4. Scroll to the **Pre-start** card (the counter reads **N TO GO**, then **ALL DONE**). It is the same card whether the job is **offered** to a contractor or **assigned** to an employee — the list belongs to the job, not to who is painting it, and it is built the moment the job is issued and again when a painter goes on it. Answer **Colour schedule finalised** with **Yes**, or **No** if any colour still needs a colour match, then tick **Materials ordered** (it needs the colours first), **Equipment movements booked** and **Access details recorded**. **Pre-start checklist** and **SWMS / induction attached** are optional on a residential job; ticking the first emails the customer the pre-start checklist, and the second is required on commercial and body corporate jobs. An item that ticks itself is marked **auto**.
   ![](media/pc-04.png)
5. When the card reads **ALL DONE** the job can start. Further down, under the job facts, tick **Quality check required** if this job needs a check regardless of the contractor's record (a new contractor's first three jobs get one automatically); **Add a mid-job check** schedules an extra one for a date you choose. **Quality check not required on this job** is the override the other way: press it for a job that should skip the check even though the contractor is new — any check already due comes off the books, the facts read **Not required — waived by the office**, and a job parked at Quality check moves on as a pass would. Ticking **Quality check required** or adding a mid-job check turns the waiver off again.
   ![](media/pc-05.png)
6. **Next step** offers **Start the job** once the list is true, with the gate's wording underneath while it is not. You can start early; the console warns that starting moves the start date to today and asks you to press again. Otherwise the job starts itself on its booked date once the list is done.
   ![](media/pc-06.png)
7. At **03 In progress** the **Scope & ticks** card shows the painter's list live, with the amber before-photo prompts until each area has its photo. Under each line is a small **Photos not required — …** button: press it on a line that is not a surface (a fuel allowance, a site set-up line) and the painter is never asked for a before or finished shot of it; the row shows **No photos** and the button reads **Photos required again**. The same button sits on the read-only scope list before the job starts. On a job booked for three days or fewer the painter needs only **one** before photo and **one** finished photo for the whole job rather than one per area; the threshold is Settings → work-order loop → photoMinimums.shortJobDays.
   ![](media/pc-07.png)
...
### Changes from Revise scope — one offer, one signature
11a. Changes made in **Revise scope** (the working scope) go to the customer as **one offer**. Edit the scope, then **Save & draft variations for signature**: the panel shows one row — "2 changes · one offer for the customer" — with the net figure (additions minus removals, inc GST) and the two changes listed under it. There is one **Copy signing link** and one **Email / Text / Both** for the whole offer; draft five changes and it is still one link. Every addition needs a photo before the offer can be sent (a removal does not); the send button stays off until each one has one.
11b. The customer opens the link, sees every change with its photos and lines, the total for the lot, and their job total before and after. They **Approve all** with one signature, or **No thanks** declines the lot — the changes are never approved one by one. Once answered, the same link shows the outcome.
11c. Where the changes go next depends on whether anyone is on the job yet. **No painter on the job** (not offered, not assigned): the signed changes are simply part of the job — they read as **In the job** on the job sheet, and the offer you send later carries the base pay plus those changes as one figure. **A painter already on the job**: each signed addition is released to them as usual (**YOUR APPROVAL** on their job page, or your **Release** step if the automation is off), and a removal is acknowledged. Either way the painter's job sheet now lists **Changes to the scope** — what was added or removed, with hours, never the customer's price — and every approved addition with site hours is also on their tick list: a new area under its own heading with a row per surface, a raised variation as one row under **Variations**. Those rows count toward progress and the finish gate like any other. A change with no hours (a price-only adjustment) adds no row.
...
17. At **05 Walkthrough** what the **Next step** card offers depends on the job. **A job that went through a quality check is signed off by the office, not the customer**: the button reads **Sign off as complete — quality check passed**, and the **Walkthrough** card carries the same button with an optional note for the record. Pressing it approves every area on the customer's behalf, signs with your own name, freezes the report, starts the warranty, drafts the final invoice and closes the job; the customer is not asked to sign and is not sent reminders — they receive the completion report the moment it is signed. The painter's page shows **Quality check passed — Paint Group signs this job off from the office** instead of the walkthrough button. On a job with **no** quality check the normal path stands: the customer signs on the painter's phone, and the Next-step button is the office closing the job on their behalf. The **Walkthrough** card shows the estimated finish and the booked **FINAL** walkthrough with **Missed** and **Cancel**; pick a date and time and **Rebook final** to change it (an empty date lands on the last day on site), or **Book pre** for a pre-walkthrough. The **Walkthrough not required** button (on the Walkthrough card from the offer stage onwards, so it can be pressed the moment a job is approved or whenever circumstances change) means the job closes at the invoice stage once it is finished and checked; it reads **Walkthrough not required ✓ — require it again** while it is on. If the job is already at the walkthrough stage when you press it, the **Next step** card changes to **Close the job — no walkthrough required**: pressing that cancels any booked walkthrough and closes the job with the same report, warranty and invoice draft a signing would write. Under **Sign-off from our side**, **Walk through on this device** runs the customer's walkthrough on your own screen when you are with them, and **Record sign-off manually** records an approval they gave by phone or on paper, with their full name and how they approved. **Customer can't attend — open remote sign-off** is the gate for the remote path: only open it when the customer genuinely cannot be there; it sends them their own signing link.
    ![](media/pc-15.png)
    ![](media/pc-16.png)

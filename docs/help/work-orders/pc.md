...
9. Quick price: enter the hours and any materials. The preview shows the customer price from the pricing engine and the contractor's amount.
   ![](media/pc-09.png)
10. Priced, the card reads "with the customer" with the amount, and the message under it says what happened to the signing link: "has been emailed", or the reason it was not (no email on the estimate, no photo attached, email not configured). **Email the link**, **Text the link** or **Both** send it again on whichever channel the customer prefers; the reply names anything that stopped it. **Set contractor amount** overrides what the painter receives. The queue card **Variation priced, customer silent** appears if they do not answer.
    ![](media/pc-10.png)
...
12. If the customer approves over the phone instead, press **Customer approved by phone — confirm on their behalf** on the priced card. Type the name of the person who gave the approval, then a note for the record (when and how they said yes). The variation moves to **CUSTOMER** exactly as a signature would, the card reads "Approved by phone — [name], recorded by the office", and the customer is sent a written confirmation of the change and the amount by email and text (whichever are on file). Their own link then reads "Approved by phone … recorded by our office" instead of asking for a signature. Only a priced variation can be confirmed this way, and only once.
...
- **The customer never received the variation link.** Read the message under the variation card — it says whether the email went, and if not, why (no email on the estimate's contact, the customer switched job alerts off in their account, the email service not configured). The link goes to the contact on the estimate, or the email the estimate was sent to, or the customer's account email, in that order. Fix the contact on the estimate, then **Email the link**, **Text the link** or **Both**. If they have already said yes on the phone, use **Customer approved by phone — confirm on their behalf** instead.

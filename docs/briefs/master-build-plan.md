...
| 3 (shared components) | **PART-DONE** | `room_type_scope_rules` v2 live and driving the plan reader; typical dimensions seeded v3 (this commit). NOT built: `storey_heights`, room tiles/cards, Settings editor for the rules — needs the room-loop brief's foundations section |
| 4 (room-loop capture) | **NOT STARTED** | brief file not on disk |
...
| 6 (P4–P7) | **PART-DONE** | `defect_prep_rates` + `exterior_envelopes` + `requires_site_check` schema exist, seeded, unused; review queue exists on /plans but NOT dollar-ordered in the builder; $150 pre-send gate NOT built; accuracy gate NOT run — **blocked on the 25-job regression set (plans + matching PaintScout estimates for the same addresses)** |

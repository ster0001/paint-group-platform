...
| Found in | Line |
|---|---|
| C0 | Six seed scripts resolve their connection from `.env.local` (production) and ignore the environment — `scripts/c1/seed.mjs:1`, `scripts/portal/seed-demo-customer.mjs:1`, `scripts/seed-demo-loop.ts:1`, `scripts/create-test-customer.ts:1`, `scripts/create-test-contractors.ts:1`, `scripts/seed-extraction-settings.ts:1`. The fix (`scripts/seed-target.mjs`) exists only on `fix/a2-02-ratecard-transaction`. Recommended as its own PR before C1. |
| C0 | `docs/briefs/commercial-estimator-analysis.md` is untracked on disk but cited by `docs/briefs/claude-code-brief-estimator-journey-v2.md:§1`. Not committed here — C0's block names only `commercial-pricing-strategy.md`. One word from Tom and it goes in. |

-- =============================================================================
-- 20270188 · import_booked_job_set_scope passes the estimate freeze
-- =============================================================================
-- 20270187's function refused on production with "accepted estimate is
-- frozen — revisions belong on the working scope": estimate_frozen_guard
-- (20270149) exempts `current_user = 'service_role'`, but inside a SECURITY
-- DEFINER function current_user is the owner, so the exemption never applied.
-- The test project never showed it because scripts/c1/apply-migrations.mjs
-- stops at 20270139, so the guard (20270149) is not installed there — the
-- e2e proved the write against a database without the freeze.
-- Same body as 20270187 plus the guard's transaction-local switch
-- (`estimates.options = on`), which is how the options RPCs edit an accepted
-- estimate. Converges on a re-run: or-replace only.
-- =============================================================================
...
select
  ...
  (select prosrc like '%estimates.options%' from pg_proc where proname = 'import_booked_job_set_scope') as fn_passes_freeze,
  ...
  'true,true,true,false' as _expect_;

insert into public._prod_migrations(name) values ('20270188000000_import_set_scope_passes_freeze.sql') on conflict (name) do nothing;

...
test("a partial or missing row renders the defaults, never throws", () => {
  expect(parseWebsiteContent(undefined)).toEqual({ painters: [], promisePhotos: [], storyPhotos: [] });
  ...
  expect(parseWebsiteContent({ painters: [{ name: "" }] })).toEqual({ painters: [], promisePhotos: [], storyPhotos: [] }); // invalid → defaults
});

...
import { openQaChecks, qaAllClear, staffSignsOff, supersededQaIds } from "./qa";
...
test("the office signs off a job that went through a quality check — a pass on record", () => {
  expect(staffSignsOff([fail, recheckPassed])).toBe(true);
  expect(staffSignsOff([{ id: "a", result: "pass" }])).toBe(true);
});

test("no pass on record (no check, waived, or still open) — the customer signs as before", () => {
  expect(staffSignsOff([])).toBe(false);
  expect(staffSignsOff([{ id: "a", result: null }])).toBe(false);
  expect(staffSignsOff([fail, recheckPending])).toBe(false);
});

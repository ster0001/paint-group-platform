...
export function checkThresholds(doc: ScopeDoc, deps: ScopeDeps) {
  ...
  if (p.decision.outcome !== "reveal") {
    return { outcome: "visit" as const, tier: "bronze" as const, reasons: [GUARDRAIL_MESSAGES[p.decision.outcome] ?? GUARDRAIL_MESSAGES.handoff], accuracyPct: p.payload.accuracyPct, minAccuracyPct: minAcc, capCents: cap, guardrail: p.decision.outcome };
  }

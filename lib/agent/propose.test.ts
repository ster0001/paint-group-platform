...
describe("co-work: the pending proposal and the apply gate", () => {
  ...
  it("propose_diff writes a PENDING proposal; the live tree stays empty until apply_diff", async () => {
    const { scope, tools, tctx } = harness();
    const r = await tools.execute("propose_diff", { text: INJECTED, sourceKind: "paste" }, tctx);
    expect(r.status).toBe("ok");
    const data = (r as { data: { injectedInstructions: string[]; unmapped: string[]; added: unknown[]; applied: boolean; priced: { totalCents: number } } }).data;
    ...
    // The injected "$500" changed nothing: the proposal prices from the card.
    ...
    expect(isBuilt(live)).toBe(false);
    expect(pendingOf(live)).not.toBeNull();
    // The panel reads the proposal back from the estimate — same fill-ins, same warning.
    const shown = pendingSummary(live, staff, settings.priceImpactGateCents)!;
    expect(shown.injectedInstructions).toHaveLength(1);
    expect(shown.unmapped).toEqual(["Also oil the deck."]);
    expect(shown.assumed.some((a) => a.label.startsWith("Assumed: two coats"))).toBe(true);
    // The unmapped item made the PROPOSAL amber, not the live estimate.
    expect(pendingOf(live)!.requiresSiteCheck).toBe(true);
    expect(live.requiresSiteCheck).toBe(false);

    // Answering a gap edits the proposal, not the live tree.
    ...
    expect(ans).toMatchObject({ status: "ok", data: { applied: true, pending: true } });
    const priced = await tools.execute("price_scope", {}, tctx);
    const pd = (priced as { data: { pending: boolean; totalCents: number; liveTotalCents: number | null } }).data;
    expect(pd.pending).toBe(true);
    expect(pd.totalCents).toBeGreaterThan(before);
    expect(pd.liveTotalCents).toBeNull();
    expect(isBuilt((await scope.load("est-1"))!)).toBe(false);

    const applied = await tools.execute("apply_diff", { diffId: "pending" }, tctx);
    expect(applied.status).toBe("ok");
    expect((applied as { data: { rows: number; totalCents: number } }).data.totalCents).toBe(pd.totalCents);
    const after = (await scope.load("est-1"))!;
    expect(isBuilt(after)).toBe(true);
    expect(pendingOf(after)).toBeNull();
    expect(after.requiresSiteCheck).toBe(true);
    // Applied rows carry their provenance, not human_confirmed.
    const origins = new Set(docBlocks(after).filter((b) => b.kind === "area").map((b) => String(b.origin)));
    expect([...origins].every((o) => o === "ai_assumed" || o === "ai_extracted")).toBe(true);
    expect((after.builderState.agent as { appliedDiffs: Array<{ by: string }> }).appliedDiffs[0].by).toBe("staff-1");
    // Re-applying with nothing pending is refused, not silently repeated.
  ...
  it("the stub co-work turn: paste → proposal with the two $/hr figures → apply", async () => {
    ...
    expect(r1.text).toMatch(/Proposed price \$[\d,]+ – \$[\d,]+/);
    expect(r1.text).toMatch(/charge-out \$\d+\/hr · revenue \$\d+\/hr/);
    expect(isBuilt((await scope.load("est-1"))!)).toBe(false);
    const r2 = await runTurn(deps, { conversationId: conv.id, text: "apply", actor: "staff" });
    expect(r2.toolCalls.map((c) => c.tool)).toContain("apply_diff");
    expect(r2.text).toMatch(/Applied — \d+ rows/);
